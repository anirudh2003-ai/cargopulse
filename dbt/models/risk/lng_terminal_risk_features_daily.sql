...
-- depends_on: {{ ref('int_lng_call_operations') }}
-- depends_on: {{ ref('lng_historical_duration_baseline_daily') }}
-- depends_on: {{ ref('lng_call_delay_metrics') }}
...
call_baselines AS (
    ...
    JOIN {{ ref('int_lng_call_operations') }}
        AS calls
    ...
    LEFT JOIN
        {{ ref('lng_historical_duration_baseline_daily') }}
        AS historical
...
completed_summary AS (
    ...
    LEFT JOIN {{ ref('lng_call_delay_metrics') }}
        AS calls
